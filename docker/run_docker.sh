#!/bin/bash
set -e
DOCKER_IMAGE_NAME='openarm_isaac_lab'
DOCKER_VERSION_TAG='latest'

SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )

WORKDIR="/workspaces/openarm_isaac_lab"
...
while getopts ":d:m:e:hn:rv" OPTION; do
    case $OPTION in
        d)
        ...
        h)
            ...
            echo "Helper script to build and run the OpenArm Isaac Lab docker environment."
            echo ""
            echo "Usage:"
