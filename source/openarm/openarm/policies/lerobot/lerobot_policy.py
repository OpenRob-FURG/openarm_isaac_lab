...
class LeRobotPolicy(PolicyBase):
    def __init__(self, repo_id, task: str, policy_type="smolvla") -> None:
        ...
        if policy_type == "smolvla":
            ...
            self.policy = GrootPolicy.from_pretrained(pretrained_name_or_path=repo_id)
    ...
    def get_action(self, env, observation):
        return self.post_processor(
            self.policy.select_action(
                self.pre_processor(
                    {
                        "observation.images.top": observation["camera_obs"].float().reshape((env.num_envs, 3, 256, 256)).to(self.policy.config.device),
